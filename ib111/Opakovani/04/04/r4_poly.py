...
def differentiate(poly: Polynomial) -> Polynomial:
    ...
    for i in range(len(poly) - 1):
        result.append(Fraction(poly[i] * (len(poly) - 1 - i)))
...
def integrate(poly: Polynomial) -> Polynomial:
    ...
    for i in range(len(poly)):
        if (len(poly) - i - 1) == 0:
            result.append(Fraction(poly[i], 1))
        else:
            result.append(Fraction(poly[i], (len(poly) - i)))
...
def check_inverse(poly: Polynomial) -> bool:
    if differentiate(integrate(poly)) != poly:
        return False

    diff = integrate(differentiate(poly))
