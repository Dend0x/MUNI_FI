...
def cellular_in_situ(state: list[int]) -> None:
    for i in range(len(state)):
        left = 4 * state[i]
        middle = 0 if i + 1 >= len(state) else 2 * state[i + 1]
        right = 0 if i + 2 >= len(state) else state[i + 2]
        comp = left + middle + right

        if comp == 0 or comp == 1 or comp == 6:
            continue
        if comp == 2 or comp == 3:
            state[i] = 1
        else:
