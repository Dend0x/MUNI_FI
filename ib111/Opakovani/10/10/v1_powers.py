...
def sum_different_powers_rec(num: int, k: int, power: int, current_sum: int, used: set[int]) -> bool:
    if num == current_sum and power - 1 == k:
        return True

    if power > k or current_sum > num:
        return False

    for i in range(round((num - current_sum) ** (1.0 / power)), -1, -1):
        if i in used:
            continue
        used.add(i)
        if sum_different_powers_rec(num, k, power + 1, current_sum + i ** power, used):
            return True
        used.remove(i)
...
def sum_different_powers(num: int, k: int) -> bool:
    return sum_different_powers_rec(num, k, 2, 0, set())
