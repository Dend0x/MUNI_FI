...
def digits_rec(max_length: int, cur_length: int, result: set[int], digit_sum: int, num: int, power: int, current_sum: int) -> None:
    if cur_length == max_length and digit_sum == current_sum:
        result.add(num)
        return
    if cur_length >= max_length:
        return

    for i in range(min(digit_sum - current_sum + 1, 10)):
        digits_rec(max_length, cur_length + 1, result, digit_sum, num + i * 10 ** power, power + 1, current_sum + i)


def digits(digit_sum: int, max_length: int) -> set[int]:
    result: set[int] = set()
    digits_rec(max_length, 0, result, digit_sum, 0, 0, 0)
    return result
