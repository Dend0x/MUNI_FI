...
if version_info.major != 3 or version_info.minor < 9:
    print( "python version 3.9 or newer is required", file = stderr )
    _exit( 1 )
...
def make_generator(): yield None
async def make_async(): pass
...
def allow_functions():
    ...
    basics = \
    {
        ...
        '__class__', '__class_getitem__', '__subclasshook__', '__sizeof__',
        '__name__', '__qualname__', '__dir__', '__doc__',
        '__getattribute__', '__setattribute__', '__delattr__', '__setattr__',
        '__new__', '__getnewargs__', '__init__', '__init_subclass__', '__del__',
        ...
        '__ceil__', '__divmod__', '__floor__', '__invert__', '__radd__',
        '__rand__', '__rdivmod__', '__rfloordiv__', '__rlshift__',
        '__rmod__', '__rmul__',
        '__ror__', '__round__', '__rpow__', '__rrshift__', '__rsub__',
        '__rtruediv__', '__rxor__', '__trunc__',
    ...
    allow[ int ] = basics | \
    {
        '__add__', '__sub__', '__mod__', '__floordiv__', '__neg__', '__abs__',
        '__mul__', '__pow__', '__float__',
        '__truediv__', # wrapped
        '__or__'       # wrapped
    }
    ...
    allow[ float ] = basics | \
    {
        '__add__', '__sub__', '__truediv__', '__neg__', '__abs__',
        '__mul__', '__pow__', '__int__',
        '__getformat__', '__setformat__',   # internal float machinery
        '__get_format__', '__set_format__', # same but for 3.9.10 and older
    }
    ...
    allow[ builtins ] = \
    {
        'min', 'max', 'abs', 'print', '__import__', 'range', 'round', 'float',
        'AssertionError', 'AttributeError',
        '__name__', # overriding builtins.__name__ makes things very confusing
        'hasattr', 'tuple', 'str', 'int', 'bool' # killed by overrides_late()
    }
    ...
    if level >= 6 and allow_data_structures:
        allow[ set ]      |= { '__or__', '__and__', '__sub__',
                               'update', 'intersection_update', 'difference_update' }
        allow[ list ]     |= { 'reverse', '__contains__', 'index', '__add__', 'extend', 'insert' }
    ...
    if level >= 11:
        ...
        allow[ str ] |= { '__getitem__', '__iter__', '__len__', '__add__',
                          'isupper', 'islower', 'isalpha', 'isdecimal',
                          'upper', 'lower', 'replace', 'strip', 'rstrip',
                          'split', 'join' }
...
def allow_modules():
    ...
    allow[ 'qold' ] = [ 'nat', 'nat1', 'nat2', 'str1', 'char', 'assert_eq', 'delay',
                        '+oneof', 'small', 'mixed', 'fixed', 'qgen', 'sequence',
                        'to_test', 'distribution', 'generator', 'fmt_value' ]
    allow[ 'quick' ] = [ 'ratf', '+anno_get_enum', '+get_bounded', 'anno_enum',
                         'tuple_enum', 'list_enum', 'dict_enum', 'set_enum',
                         'get_str_enum', 'str_enum', 'int_enum', 'permute_enum',
                         'tuple_param', 'tuple_count', 's_exp', 's_gamma',
                         's_mult' ] + allow[ 'qold' ]
    allow[ 'turtle' ] = [ '+forward', '+backward', '+right', '+left', '+setheading',
                          '+done', '+speed', '+delay', '+penup', '+pendown' ]
    ...
    allow[ 'math' ] = \
    [
        'pi', 'acos', 'cos', 'asin', 'sin', 'atan', 'atan2', 'tan', # trig
        'sqrt', 'isqrt',                                            # roots
        'degrees', 'radians',                                       # angles
        'trunc', 'floor', 'ceil',                                   # rounding
        'isclose'
    ]
...
for _ in _async.__await__(): pass # shut up a warning

import ast
import os
import ctypes
from sys import settrace, gettrace, modules, exit, flags, \
                exc_info, getrecursionlimit
...
def is_dunder( name ):
    return _str_startswith( name, "__" ) and _str_endswith( name, "__" ) and \
           name != '__format__' and name != '__copy__' and name != '__getstate__'
    ...
    if _id( klass ) == _id( builtins ):
        ...
        for patch_ in _gen_iter( _dict_getitem( dunders, pyattr )( cklass ) ):
            ...
            if patch_obj.__class__ is struct_sequence:
                if klass is _tuple and attr != '__contains__' or attr == '__subscript__':
                    continue
...
def wrap( klass, attr, func ):
    ...
    patched = patch( klass, attr,
                     func if _isinstance( func, _type ) else with_patched )
...
def error_print( trace, *args ):
    ...
    for frame in _reversed( trace ):
        os.write( 2, bytes( f"    {frame.filename}, line {frame.lineno} in {frame.function}\n", 'utf8' ) )
...
def wrap_import( attr, old, name, *args, **kwargs ):
    with overrides_disabled():
        ...
        if flags.interactive and name in [ 'sys', 'atexit', 'readline', 'rlcompleter', 'pydoc' ]:
...
def check_import():
    ...
    matched = fullmatch( f"from {__name__} import week_(..)"
                          "(, except_data_structures|)(  # noqa|)\n", line )

    if matched is None:
        _error( f"wrong import of {__name__}: {line}" + \
...
def check_ast( node, in_def = False, in_class = False, in_assign = False ):
    ...
    if node_is( ast.Attribute ):
        if node.attr.startswith( '__' ) and \
           ( node.attr.endswith( "__" ) or
             not isinstance(node.value, ast.Name) or node.value.id not in [ 'self', 'other' ] ):
            bad( 'using names that start with __ is not allowed' )
