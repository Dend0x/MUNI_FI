...
def bfs(graph: Graph, start: int) -> None:
    ...
    while (len(queue) != 0):
        elem: int = queue.popleft()

        for v in range(graph.size):
            if not is_edge(graph, elem, v):
                continue
            if graph.visited[v]:
                continue
            graph.discovery_time[v] = time
            graph.parent[v] = elem
            graph.distance[v] = graph.distance[elem] + 1
            graph.visited[v] = True
            time += 1

            queue.append(v)
        
        graph.finishing_time[elem] = time
    
    graph.finishing_time[start] = time

def dfs_visit(graph: Graph, u: int, time: int) -> int:
    graph.discovery_time[u] = time
    graph.visited[u] = True
    time += 1

    for v in range(graph.size):
        if not is_edge(graph, u, v):
            continue
        if graph.visited[v]:
            continue
        graph.parent[v] = u
