...
def insert_rec(node: Node | None, key: Any) -> Node | None:
    if node is None:
        return None
    ...
    if node.key > key:
        return insert_rec(node.left, key)
    else:
        return insert_rec(node.right, key)

def insert(tree: BinarySearchTree, key: Any) -> None:
    """Vlozi novy uzel s klicem 'key' do stromu 'tree'."""
    
    node: Node | None = insert_rec(tree.root, key)
...
def search_rec(node: Node | None, key: Any):
    if node is None:
        return None
...
def transplant(tree: BinarySearchTree, node1: Node | None, node2: Node | None) -> None:
...
def delete(tree: BinarySearchTree, node: Node) -> None:
    ...
    if node.left is None:
        ...
        if node.right is None:
            ...
            newNode: Node | None = minimum(node.right)
...
def height_rec(node: Node | None)-> int:
    ...
    return max(height_rec(node.left), height_rec(node.right)) + 1
...
def is_correct_bst_rec(node: Node | None, low: int, high: int) -> bool:
    if node is None:
        return True

    if node.key < low or node.key > high:
        return False
    if not is_correct_bst_rec(node.left, low, node.key):
        return False
    if not is_correct_bst_rec(node.right, node.key, high):
        return False
    return True
...
def is_correct_bst(tree: BinarySearchTree) -> bool:
    ...
    """
    return is_correct_bst_rec(tree.root, float('-inf'), float('inf'))
