...
def parent(heap: MinHeap, i: int) -> Any | None:
    ...
    """
    # TODO
    par_i: int|None = parent_index(i)
    if par_i == None:
        return None
    return heap.array[par_i]
...
def heapify(heap: MinHeap, i: int) -> None:
    ...
    # TODO
    smallest = i
    left_son = left(heap, i)
    right_son = right(heap, i)
    if left_son != None and left_son < heap.array[smallest]:
        smallest = left_index(i)
    if right_son != None and right_son < heap.array[smallest]:
...
def decrease_key(heap: MinHeap, i: int, value: Any) -> None:
    ...
    heap.array[i] = value
    par = parent_index(i)
    while i > 0 and heap.array[par] > heap.array[i]:
        swap(heap, i, par)
        i = par
        par = parent_index(i)
