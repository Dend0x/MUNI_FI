...
def print_list(start: Node) -> None:
    print(start.value, " -> ", start.next.value)
    current: Node = start.next
    while current != start:
...
def print_jump(start: Node) -> None:
    assert start.jump is not None
    print(start.value, " -> ", start.jump.value)
    current: Node = start.next
    while current != start:
        assert current.jump is not None
        print(current.value, " -> ", current.jump.value)
        current = current.next
    print("------------------------------------------")
