...
def parent(heap: MinHeap, i: int) -> Any | None:
    ...
    Pokud neexistuje, vrati None.
    """
    index: int | None = parent_index(i)
    return None if index is None or index == None else heap.array[index]
...
def left(heap: MinHeap, i: int) -> Any | None:
    ...
    Pokud neexistuje, vrati None.
    """
    index: int | None = left_index(i)
    return None if index is None or index >= heap.size else heap.array[index]
...
def heapify(heap: MinHeap, i: int) -> None:
    ...
    """
    largest: int = i
    left_v: int | None = left(heap, i)
    right_v: int | None = right(heap, i)
    if (left_v is not None and left_v < heap.array[largest]):
        largest = left_index(i)
    if (right_v is not None and right_v < heap.array[largest]):
        largest = right_index(i)
    if largest != i:
        swap(heap, i, largest)
...
def helper_test_extract_min(heap: MinHeap) -> bool:
    ...
    if heap.array != [3, 5, 4] or tmp != 2:
        print("NOK - chyba ve funkci extract_mina")
        return False

    tmp = extract_min(heap)
    if heap.array != [4, 5] or tmp != 3:
        print("NOK - chyba ve funkci extract_minb")
        return False

    tmp = extract_min(heap)
    if heap.array != [5] or tmp != 4:
        print("NOK - chyba ve funkci extract_minc")
        return False

    tmp = extract_min(heap)
    if heap.array != [] or tmp != 5:
        print("NOK - chyba ve funkci extract_mind")
        return False

    try:
        if extract_min(heap) is not None:
            print("NOK - chyba ve funkci extract_mine")
            return False
    except Exception:
        print("NOK - chyba ve funkci extract_min na prazne haldef")
        return False
